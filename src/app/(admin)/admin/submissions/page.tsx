...
export default function AdminSubmissionsPage() {
  ...
  async function handleAction(id: number, type: string, action: "approve" | "decline") {
    const key = `${type}-${id}`;
    setActionId(key);
    setMessage(null);
    try {
      const res = await fetch(`/api/admin/submissions/${id}`, {
        ...
        body: JSON.stringify({ action, submissionType: type }),
      });
      if (res.ok) {
        setMessage({ type: "success", text: `Submission ${action}d.` });
        fetchItems();
      } else {
  ...
  return (
    <div>
      <h1 className="font-serif text-3xl text-forest font-bold mb-2">Submissions</h1>
      <p className="text-gray-600 mb-8">Review article, video, and book submissions from members.</p>
      ...
      {loading ? <p className="text-gray-500">Loading...</p> : (
        <>
          ...
          {pending.length === 0 ? (
            <Card className="mb-8"><p className="text-gray-500 text-center py-4">No pending submissions.</p></Card>
          ) : (
            <div className="space-y-3 mb-8">
              {pending.map((item) => (
                <Card key={`${item.type}-${item.id}`}>
                  ...
                  <p className="text-gray-600 text-sm mb-4 line-clamp-3">{item.detail}</p>
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      loading={actionId === `${item.type}-${item.id}`}
                    ...
                    <Button
                      ...
                      variant="danger"
                      loading={actionId === `${item.type}-${item.id}`}
                      onClick={() => handleAction(item.id, item.type, "decline")}
                    >
                      Decline
                    </Button>
                  </div>
